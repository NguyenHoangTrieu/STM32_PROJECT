...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomMy_ServiceHdle;                    /**< My_Service_1 handle */
  uint16_t  CustomTest_ValHdle;                  /**< Test_Value handle */
...
#define COPY_MY_SERVICE_1_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_TEST_VALUE_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x01,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomTest_ValHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_TEST_VAL_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_TEST_VAL_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
        ...
        case ACI_GATT_READ_PERMIT_REQ_VSEVT_CODE :
          ...
          if (read_req->Attribute_Handle == (CustomContext.CustomTest_ValHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for My_Service_1 +
   *                                2 for Test_Value +
   *                                1 for Test_Value configuration descriptor +
   *                              = 4
   *
  ...
  COPY_MY_SERVICE_1_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomMy_ServiceHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: My_Service, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: My_Service \n\r");
  }

  /**
   *  Test_Value
   */
  COPY_TEST_VALUE_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomMy_ServiceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeTest_Val,
                          ...
                          &(CustomContext.CustomTest_ValHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : TEST_VAL, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : TEST_VAL \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_TEST_VAL:
      ret = aci_gatt_update_char_value(CustomContext.CustomMy_ServiceHdle,
                                       CustomContext.CustomTest_ValHdle,
                                       0, /* charValOffset */
                                       SizeTest_Val, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value TEST_VAL command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value TEST_VAL command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_TEST_VAL:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMy_ServiceHdle, CustomContext.CustomTest_ValHdle, SizeTest_Val, pPayload);
